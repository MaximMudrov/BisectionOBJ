...
namespace triang
{
	...
	{
	private:
		Point* _AuxiliaryDots[INIT_VERTICES_COUNT];
		std::vector<Point*>* _ProjectedDots;
		...
		void BuildInitialHull(std::vector<Point*>* dots);
		void InsertDot(Point* dot);
		void RemoveExtraTriangles();
		void SplitTriangle(Triangle* triangle, Point* dot);
		...
		double GetDistance(Point* v0, Point* v1);
		double GetDeterminant(Point* v0, Point* v1, Point* v2);
	...
	public:
		...
		std::vector<std::tuple<int, int, int>*> GetTriangulationResult(std::vector<Point*>& dots);
